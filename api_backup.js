...
<body>
    ...
    async searchCombined(textQuery = null, headerName = null, headerValue = null, titleQuery = null, limit = 50, offset = 0) {
        ...
        function displayResults(data) {
            ...
            data.results.forEach(result => {
                ...
                html += '<div class="result">' +
                        ...
                        '<div class="result-title">' + displayTitle + '</div>' +
                        '<div class="result-domain">🌐 <a href="#" class="domain-link" data-domain="' + result.domain + '" style="color: #00bfff; text-decoration: none;">' + result.domain + '</a></div>' +
                        (headerSnippet ? '<div class="result-headers">' + headerSnippet + '</div>' : '') +
                        '<div class="result-content">' + displayContent + '</div>' +
                        '<div class="result-meta">' +
                            '<span>📅 ' + new Date(result.last_crawled).toLocaleDateString() + '</span>' +
            ...
            // Add event listeners for domain links
            document.querySelectorAll('.domain-link').forEach(link => {
                link.addEventListener('click', function(e) {
                    e.preventDefault();
                    showDomainPage(this.getAttribute('data-domain'));
                });
            });
        }
        ...
        async function loadDomainLinks(domain) {
                ...
                if (data.incomingLinks.length === 0) {
                    ...
                    data.incomingLinks.forEach(function(link) {
                        incomingHtml += '<div class="result" style="margin-bottom: 10px;">' +
                            ...
                            '</div>' +
                            '<div style="font-size: 0.9em;">' +
                            'From: <a href="#" class="domain-link" data-domain="' + link.source_domain + '" style="color: #00bfff;">' + link.source_domain + '</a>' +
                            '</div>' +
                            '<div style="font-size: 0.85em; color: #64748b;">' +
                ...
                if (data.outgoingLinks.length === 0) {
                    ...
                    data.outgoingLinks.forEach(function(link) {
                        outgoingHtml += '<div class="result" style="margin-bottom: 10px;">' +
                            ...
                            '<div style="font-size: 0.9em;">' +
                            'To: ' + (link.target_domain ? 
                                '<a href="#" class="domain-link" data-domain="' + link.target_domain + '" style="color: #00bfff;">' + link.target_domain + '</a>' :
            ...
            // Add event listeners for domain links in the loaded content
            document.querySelectorAll('.domain-link').forEach(link => {
                link.addEventListener('click', function(e) {
                    e.preventDefault();
                    showDomainPage(this.getAttribute('data-domain'));
