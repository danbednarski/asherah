...
export class TcpScanner {
  ...
  private readonly bannerTimeout: number;
  ...
  async scanPort(domain: string, port: number, probeString?: string): Promise<PortScanResult> {
  ...
  private async grabBanner(socket: Socket, probeString?: string): Promise<string | null> {
  ...
  private classifyError(errorMessage: string): 'closed' | 'filtered' | 'timeout' {
