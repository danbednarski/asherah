...
export function safeRenderHighlightedText(text) {
    if (!text)
        return '';
    const escaped = escapeHtml(text);
    return escaped.replace(/&lt;(\/?)mark&gt;/gi, '<$1mark>');
}
export function findBestSnippet(content, parsedQuery) {
    if (!content)
        return 'No content available';
    const maxSnippetLength = 300;
    const searchTerms = [];
    if (parsedQuery.text) {
        searchTerms.push(...parsedQuery.text
            .toLowerCase()
            .split(' ')
            .filter((term) => term.length > 2));
    }
    if (parsedQuery.title) {
        searchTerms.push(...parsedQuery.title
            .toLowerCase()
            .split(' ')
            .filter((term) => term.length > 2));
    }
...
export function highlightSearchTerms(text, parsedQuery) {
    if (!text)
        return '';
...
export function createHeaderSnippet(headers, parsedQuery) {
...
export function renderSearchResults(query, searchResults, parsedQuery) {
    if (!searchResults)
        return '';
    ...
    const filters = [];
    if (parsedQuery?.title) {
        filters.push(`📝 Title contains: <code style="background: rgba(0,0,0,0.5); padding: 2px 6px; border-radius: 3px;">${escapeHtml(parsedQuery.title)}</code>`);
    }
    if (parsedQuery?.header || parsedQuery?.value) {
        filters.push(`📊 HTTP header: <code style="background: rgba(0,0,0,0.5); padding: 2px 6px; border-radius: 3px;">${escapeHtml(parsedQuery?.header ?? 'any')}: ${escapeHtml(parsedQuery?.value ?? 'any')}</code>`);
    }
    if (parsedQuery?.port) {
        filters.push(`🔌 Open port: <code style="background: rgba(0,0,0,0.5); padding: 2px 6px; border-radius: 3px;">${parsedQuery.port}</code>`);
    }
    if (filters.length > 0) {
...
export function getHTMLInterface(query, searchResults, parsedQuery, error, stats) {
...
export function getDomainPageHTML(domainInfo, incomingLinks, outgoingLinks, _pagination) {
