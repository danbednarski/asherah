import pg from 'pg';
import { DomainUpsertResultSchema, PageUpsertResultSchema, QueueItemSchema, StatisticsRowSchema, DomainStatusRowSchema, DomainLockResultSchema, DomainReleaseResultSchema, DomainExtendResultSchema, SearchResultRowSchema, DomainDetailsRowSchema, IncomingLinkRowSchema, OutgoingLinkRowSchema, } from '../schemas/index.js';
...
export class Database {
    ...
    async upsertPage(domainId, url, pageData) {
        const { title, contentText, contentHtml, statusCode, contentLength, contentType, language, metaDescription, h1Tags, path, } = pageData;
        const queryText = `
      INSERT INTO pages (
    ...
    async insertLinks(pageId, links) {
        if (!links || links.length === 0)
            return;
        const values = links
            .map((_, index) => `($1, $${index * 5 + 2}, $${index * 5 + 3}, $${index * 5 + 4}, $${index * 5 + 5}, $${index * 5 + 6})`)
            .join(',');
        const params = [pageId];
    ...
    async markDomainConnectionFailed(domain, error) {
    ...
    `;
        ...
        await this.query(`UPDATE domains SET crawl_status = 'failed', is_active = false WHERE domain = $1`, [domain]);
    ...
    async acquireDomainLockAndSetCrawling(domain, workerId) {
        const result = await this.query('SELECT acquire_domain_lock_and_set_crawling($1, $2) as acquired', [domain, workerId]);
        const row = result.rows[0];
        if (!row) {
            return false;
        }
    ...
    async releaseDomainLockAndSetCompleted(domain, workerId) {
        const result = await this.query('SELECT release_domain_lock_and_set_completed($1, $2) as released', [domain, workerId]);
        const row = result.rows[0];
        if (!row) {
            return false;
        }
    ...
    async claimUrl(url, workerId) {
        await this.query("UPDATE crawl_queue SET worker_id = $2, status = 'processing', last_attempt = NOW(), attempts = attempts + 1 WHERE url = $1 AND status = 'pending'", [url, workerId]);
    }
    ...
    async searchCombined(textQuery = null, headerName = null, headerValue = null, titleQuery = null, limit = 50, offset = 0, port = null, path = null) {
        let textCondition = '';
        let headerCondition = '';
        let titleCondition = '';
        let portCondition = '';
    ...
    // ==================== Scanner Methods ====================
    async getNextScans(workerId, limit = 1) {
        const queryText = `
    ...
    async returnScanToQueue(scanId) {
        const queryText = `
      UPDATE scan_queue
    ...
    async markScanCompleted(scanId) {
        const queryText = `
      UPDATE scan_queue
    ...
    async markScanFailed(scanId, error) {
        const queryText = `
      UPDATE scan_queue
      SET status = CASE
        WHEN attempts >= 3 THEN 'failed'
    ...
    async getPortScanId(domain, port) {
    ...
    async insertDetectedService(portScanId, domain, port, serviceName, serviceVersion, confidence, rawBanner) {
        const queryText = `
    ...
    async addToScanQueue(domain, profile = 'standard', ports = null, priority = 100) {
    ...
    async getPortScanResults(domain) {
        const queryText = `
      SELECT
        ps.port,
        ps.state,
    ...
    async populateScanQueueFromDomains(limit = 1000, profile = 'standard') {
    ...
    async queueDomainForScan(domain, priority = 100) {
    ...
    // ==================== Dir Scanner Methods ====================
    async getNextDirScans(workerId, limit = 1) {
        const queryText = `
      WITH available_scans AS (
        SELECT dq.id
    ...
    async returnDirscanToQueue(scanId) {
        const queryText = `
      UPDATE dirscan_queue
      SET status = 'pending', worker_id = NULL
      WHERE id = $1
    ...
    async markDirscanCompleted(scanId) {
        const queryText = `
      UPDATE dirscan_queue
      SET status = 'completed', worker_id = NULL
      WHERE id = $1
    ...
    async markDirscanFailed(scanId, error) {
        const queryText = `
      UPDATE dirscan_queue
      SET status = CASE
        WHEN attempts >= 3 THEN 'failed'
    ...
    async populateDirscanQueueFromDomains(limit = 1000, profile = 'standard') {
